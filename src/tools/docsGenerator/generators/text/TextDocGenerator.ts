...
export class TextDocGenerator extends DocGeneratorBase implements IDocGenerator {
    generateDoc(packageConfig: ImportsBetweenPackagesRuleConfig): void {
        ...
        packageConfig.checkImportsBetweenPackages.packages
            .filter(pkg => !pkg.isExternal && this.filter.isImportPathOkForFolderWithWhitelist(pkg))
            .forEach(pkg => {
    ...
    private outputSubFolders(subFolders: PackageSubFolder[]) {
        ...
        subFolders.forEach(folder => {
            ...
            const allowedToImport = this.outputAllowedToImport(folder.allowedToImport);
